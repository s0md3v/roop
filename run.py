...
parser.add_argument(
    "-o", "--output", help="save output to this file", dest="output_file"
)
parser.add_argument(
    "--gpu", help="use gpu", dest="gpu", action="store_true", default=False
)
parser.add_argument(
    "--keep-fps",
...
parser.add_argument(
    "--max-memory", help="maximum amount of RAM in GB to be used", type=int
)
parser.add_argument(
    "--max-cores",
    help="number of cores to be use for CPU mode",
    dest="cores_count",
    type=int,
    default=max(psutil.cpu_count() - 2, 2),
)
    ...
    if "--gpu" in sys.argv:
        ...
        if len(list(set(core.globals.providers) - set(NVIDIA_PROVIDERS))) == 1:
            ...
            if not torch.cuda.is_available() or not CUDA_VERSION:
                quit(
                    "You are using --gpu flag but CUDA isn't available or properly installed on your system."
                )
            if CUDA_VERSION > "11.8":
                quit(
                    f"CUDA version {CUDA_VERSION} is not supported - please downgrade to 11.8"
                )
            if CUDA_VERSION < "11.4":
                quit(
                    f"CUDA version {CUDA_VERSION} is not supported - please upgrade to 11.8"
                )
            if CUDNN_VERSION < 8220:
                quit(
                    f"CUDNN version {CUDNN_VERSION} is not supported - please upgrade to 8.9.1"
                )
            if CUDNN_VERSION > 8910:
                quit(
                    f"CUDNN version {CUDNN_VERSION} is not supported - please downgrade to 8.9.1"
                )
    else:
...
def save_file():
    ...
    args["output_file"] = asksaveasfilename(
...
def start():
    ...
    if is_img(target_path):
        ...
        return
    seconds, probabilities = predict_video_frames(
        video_path=args["target_path"], frame_interval=100
    )
    ...
    save_path = (
        args["output_file"]
        if args["output_file"]
        else output_dir + "/" + video_name + ".mp4"
    )
...
if __name__ == "__main__":
    ...
    support_link.bind(
        "<Button-1>", lambda e: webbrowser.open("https://github.com/sponsors/s0md3v")
    )
    ...
    fps_checkbox = tk.Checkbutton(
        ...
        selectcolor="black",
        text="Limit FPS to 30",
        fg="#dfe6e9",
        borderwidth=0,
        highlightthickness=0,
        bg="#2d3436",
        variable=limit_fps,
        command=toggle_fps_limit,
    )
    fps_checkbox.place(x=30, y=500, width=240, height=31)

    # Keep frames checkbox
    keep_frames = tk.IntVar(None, args["keep_frames"])
    frames_checkbox = tk.Checkbutton(
        window,
        relief="groove",
        activebackground="#2d3436",
        activeforeground="#74b9ff",
        selectcolor="black",
        text="Keep frames dir",
        fg="#dfe6e9",
        borderwidth=0,
        highlightthickness=0,
        bg="#2d3436",
        variable=keep_frames,
        command=toggle_keep_frames,
    )
    ...
    start_button = tk.Button(
        ...
        borderwidth=0,
        highlightthickness=0,
        command=lambda: [save_file(), start()],
    )
