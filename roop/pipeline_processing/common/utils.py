...
def run_ffmpeg(args, level = 'error'):
    log_level = f'-loglevel {level}'
...
def create_video(video_name, fps, output_dir, vendor):
    hwaccel_option = '-hwaccel cuda' if vendor == 'nvidia' else ''
...
def extract_frames(input_path, output_dir, vendor):
    hwaccel_option = '-hwaccel cuda' if vendor == 'nvidia' else ''
