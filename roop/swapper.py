...
def process_frames(source_path: str, temp_frame_paths: List[str], progress=None) -> None:
    ...
    for temp_frame_path in temp_frame_paths:
        ...
        try:
            ...
            state.mark_frame_processed(temp_frame_path)
...
def process_video(source_path: str, temp_frame_paths: List[str], mode: str) -> None:
    ...
    with tqdm(total=len(temp_frame_paths) + len(state.state_struct['frames']), desc='Processing', unit='frame', dynamic_ncols=True, bar_format=progress_bar_format) as progress:
        if mode == 'cpu':
